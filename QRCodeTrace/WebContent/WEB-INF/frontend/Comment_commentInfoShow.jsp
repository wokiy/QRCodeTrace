    ...
    <div class="header_resize">
      <div class="menu_nav">
        <ul>
          ...
          <li class="active"><a href="${rootPath}/Comment_commentInfoShow">留言反馈</a></li>
        </ul>
      </div>
      <div class="clr"></div>
    ...
    <div class="content_resize">
      <div class="mainbar">
          <div class="article_vert">
              ...
              <div class="comment">
                      <img src="${rootPath}/images/userpic.gif" width="40" height="40" alt="user" class="userpic" />
                  <p>
                      昵称：<s:property value="#comment.username" /><br />
                      产品：<s:property value="#comment.product.name" /><br />
                      <s:date name="#comment.date" format="yyyy-MM-dd HH:mm:ss" /></p>
                  <p>
                      <s:property value="#comment.content"/> </p>
              </div>
              </s:iterator>
              </s:if>
              <s:else>
                          ...
                          <td style="font-size: 12px; height: 40px; text-align: right; color: #696969; font-weight: bold;">
                              <s:if test="#request.pageRow >= #request.totalRow"></s:if>
                                  <s:elseif test="#request.pageNo <= 1 || #request.pageNo==null">
	                                ...
	                                <a href="${rootPath}/Comment_commentInfoShow?pageNo=<s:property value='#pageNo + 1'/>">下一页</a>
	                                <a href="${rootPath}/Comment_commentInfoShow?pageNo=<s:property value='#totalPage'/>">尾页</a>
		                             <s:form name="pageNoForm" action="Comment_commentInfoShow" style="display:inline-block" theme="simple" onsubmit="return checkPage()">
                    			  ...
                    			  <s:elseif test="#request.pageNo >= #request.totalPage">
	                                ...
	                                &nbsp; <span style="font-size: 12px;">10</span>条/页 
	                                <a href="${rootPath}/Comment_commentInfoShow?pageNo=<s:property value='1'/>">首页</a>
	                                <a href="${rootPath}/Comment_commentInfoShow?pageNo=<s:property value='#totalPage-1'/>">上一页</a>
		                             <s:form name="pageNoForm" action="Comment_commentInfoShow" style="display:inline-block" theme="simple" onsubmit="return checkPage()">
		                                 <input id="pageNo" name="pageNo" type="text" maxlength="6" style="border-color: #A0B52E; border-width: 1px;
		                                     border-style: Solid; width: 20px;"/>
		                                 <input type="submit" value="跳转"/>
          ...
          <div class="article">
          <h2>发表留言</h2>
          <s:form action="Comment_commentSave" method="post" id="leavereply" name="commentForm" theme="simple" onsubmit="return checkComment()">
          <s:token></s:token>
	          <ol><li>
	            <label for="name">昵称</label>
	            <input id="name" name="username" class="text" /><span id="nameTip"></span>
	          </li><li>
	            <label for="email">Email（可选）</label>
	            <input id="email" name="email" class="text" />
	          </li><li>
	            <label for="website">产品追溯码</label>
	            <input id="website" name="pro_id" class="text" /><span id="codeTip"></span><span style="color:red"><s:property value="#session.commentMsg"/></span>
	          </li><li>
	            <label for="message">留言内容</label>
	            <textarea id="message" name="content" rows="8" cols="50"></textarea><span id="commentTip"></span>
	          </li><li>
	            <input type="image" id="submit" src="${rootPath}/images/submit.jpg" class="send"/>
	            <input type="image" id="reset" src="${rootPath}/images/reset.jpg" class="send" onclick="reset();return false;" />
	            <div class="clr"></div>
	          </li></ol>
          </s:form>
        </div>
...
<%
	pageContext.getSession().removeAttribute("commentMsg");
